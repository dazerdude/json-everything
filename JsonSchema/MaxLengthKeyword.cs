...
namespace Json.Schema
{
	...
	public class MaxLengthKeyword : IJsonSchemaKeyword, IEquatable<MaxLengthKeyword>
	{
		...
		public void Validate(ValidationContext context, in JsonElement target, out ValidationResult result)
		{
			...
			var length = new StringInfo(target.GetString()).LengthInTextElements;
			result = ValidationResult.Check(Value >= length, $"Value is not shorter than or equal to {Value} characters");
